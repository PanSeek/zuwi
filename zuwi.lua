...
show_imgui_fullaiming					= imgui.ImBool(mainIni.weapon.fullAiming),
...
lang_menu 								= imgui.ImBool(mainIni.menu.language_menu), --0/1 - eng/rus
lang_chat 								= imgui.ImBool(mainIni.menu.language_chat), --0/1 - eng/rus
lang_dialogs 							= imgui.ImBool(mainIni.menu.language_dialogs), --0/1 - eng/rus
	...
	if array.main_window_state.v then
		...
		elseif act1 == 3 then --WEAPON IMGUI
			...
			if array.lang_menu.v then
				...
				imgui.ToggleButton('45', 'Полное прицеливание', array.show_imgui_fullaiming)
			elseif not array.lang_menu.v then
				imgui.ToggleButton('18', 'Infinity ammo and no reload', array.show_imgui_infAmmo)
				imgui.ToggleButton('45', 'Full aiming', array.show_imgui_fullaiming)
			end
			imgui.EndChild()
						 ...
						 if inicfg.save(mainIni, 'zuwi.ini') then
							mainIni = {
								...
								weapon = {
									...
									fullAiming = array.show_imgui_fullaiming.v
								},
								misc = {
	...
	--https://raw.githubusercontent.com/PanSeek/zuwi/master/version.txt
		if array.checkupdate.v then
			async_http_request('GET', 'https://raw.githubusercontent.com/PanSeek/zuwi/master/version.txt', nil, function(response)
				...
				elseif version_script > response.text then
					if array.lang_chat.v then sampAddChatMessage(tag..'Вы используете {F9D82F}тестовую {888EA0}версию скрипта', main_color) --{0984D2}
					elseif not array.lang_chat.v then sampAddChatMessage(tag..'You are using a {F9D82F}test {888EA0}version of the script', main_color) end
				end
			 	end,
			 	function(err)
		...
		elseif not array.checkupdate.v then sampAddChatMessage(tag..'У Вас выключено автообновление. Вы используете версию: {F9D82F}'..version_script, main_color) end
...
-----------------VEHICLE--------------------
	...
	if array.show_imgui_fullaiming.v then
	...
	if array.show_imgui_antibhop.v then ABHtext = '{29C730}Anti-BHop' else ABHtext = '{B22C2C}Anti-BHop' end --text antibhop for infoBar

	if isPlayerPlaying(PLAYER_HANDLE) and not isPauseMenuActive() and array.infbar.v then --infobar
...
function cmd_update()
	...
	if version_script == response.text then
		...
		elseif not array.lang_chat.v then sampAddChatMessage(tag..'You are using {0E8604}the current {888EA0}version of the script', main_color) end
	elseif version_script ~= response.text then
		if array.lang_chat.v then sampAddChatMessage(tag..'{0E8604}Обновляемся..', main_color)
		elseif not array.lang_chat.v then sampAddChatMessage(tag..'{0E8604}Updating..', main_color) end
		downloadUrlToFile('https://raw.githubusercontent.com/PanSeek/zuwi/master/zuwi.lua', 'moonloader/zuwi.lua', function(id, status)
			...
			elseif status == dlstatus.STATUS_ENDDOWNLOADDATA then
				if array.lang_chat.v then sampAddChatMessage(tag..'Загрузка обновления {0E8604}завершена', main_color)
				elseif not array.lang_chat.v then sampAddChatMessage(tag..'Download update {0E8604}complete', main_color) end
				thisScript():reload()
			end
		end)
	end
